...
import { RETAINER_TIERS, type RetainerTier, type RetainerTierId } from '../data/pricing-strategy';
import { WHATSAPP_NUMBER } from '../lib/site-config';
import { cn } from '../lib/utils';

type RetainerPricingCardsProps = {
  showSystemExamples?: boolean;
  /** Overrides tier.highlighted from data (e.g. homepage features Essential for SMMEs). */
  featuredTierId?: RetainerTierId;
  featuredBadge?: string;
};

function tierIsFeatured(tier: RetainerTier, featuredTierId?: RetainerTierId): boolean {
  if (featuredTierId) return tier.id === featuredTierId;
  return Boolean(tier.highlighted);
}
...
export function RetainerPricingCards({
  showSystemExamples = true,
  featuredTierId,
  featuredBadge = 'Most common',
}: RetainerPricingCardsProps) {
  return (
    <div className="grid lg:grid-cols-3 gap-5 lg:gap-6">
      {RETAINER_TIERS.map((tier) => {
        const featured = tierIsFeatured(tier, featuredTierId);
        return (
          <article
            key={tier.id}
            className={cn(
